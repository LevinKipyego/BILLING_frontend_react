...
export default function PppoeCredentialPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const res = await listPPPoECredentials();
      setData(res);
    } finally {
      setLoading(false);
    }
  ...
  // Sync to page 1 automatically when input parameters fluctuate
  useEffect(() => {
    setCurrentPage(1);
  }, [search, statusFilter]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-gray-900 p-4 lg:p-8 font-['Figtree',sans-serif] transition-colors duration-500">
        ...
        <div className="flex items-center gap-3">
          <div>
            <h2 className="text-xl font-black text-gray-900 dark:text-white tracking-tight leading-none">
              PPPoE <span className="text-blue-600">Credentials</span>
            </h2>
      ...
      {/* RESTRUCTURED SCANNABLE PAGINATION SECTION */}
      <div className="mt-10 flex flex-col sm:flex-row items-center justify-between gap-4 px-2">
        ...
        <div className="flex items-center gap-1.5 max-w-full justify-center">
          ...
          <div className="flex flex-wrap items-center gap-1 justify-center max-w-full">
            {(() => {
              const pages = [];
              const range = 1; // Active item threshold offset boundary
