...
export default function DashboardLayout() {
  ...
  return (
    // Main Wrapper: bg changes from light gray to deep slate/black
    <div className="flex h-screen bg-[#F8FAFC] dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300 overflow-hidden font-sans">
      
      {/* 1. MOBILE OVERLAY */}
      {sidebarOpen && (
        <div 
          className="fixed inset-0 bg-gray-900/40 dark:bg-black/60 backdrop-blur-sm z-40 lg:hidden" 
      ...
      >
        ...
        <header className="h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-gray-200 dark:border-slate-800 flex items-center justify-between px-4 md:px-8 sticky top-0 z-30 transition-colors">
          <div className="flex items-center">
            <button 
              className={`p-2 mr-4 rounded-lg transition-colors ${sidebarOpen ? "text-blue-600 bg-blue-50 dark:bg-blue-500/10" : "text-gray-600 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-800"}`} 
              onClick={() => setSidebarOpen(!sidebarOpen)}
            >
              <Bars3Icon className="w-6 h-6" />
            </button>
            <h2 className="hidden sm:block text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-[0.2em]">Network Ops</h2>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="relative" ref={notifRef}>
              ...
              {notifMenuOpen && (
                <div className="absolute right-0 mt-3 w-80 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-slate-800 overflow-hidden z-50">
                  ...
                  <NotificationItem icon={CheckCircleIcon} color="text-emerald-500" bg="bg-emerald-50 dark:bg-emerald-500/10" title="Payment Success" time="1 hour ago" desc="M-Pesa Ref: SBX092... received." />
                </div>
              )}
            </div>
            ...
            <div className="relative" ref={userRef}>
              <button 
                ...
                className={`flex items-center gap-2 p-1 pr-3 rounded-full transition-all border ${userMenuOpen ? "bg-gray-100 dark:bg-slate-800 border-gray-300 dark:border-slate-600" : "hover:bg-gray-50 dark:hover:bg-slate-800 border-transparent hover:border-gray-200 dark:hover:border-slate-700"}`}
              >
                <UserCircleIcon className="w-8 h-8 text-gray-400 dark:text-slate-500" />
                <span className="hidden lg:block text-sm font-semibold text-gray-700 dark:text-slate-300">Vendor Admin</span>
              </button>
              {userMenuOpen && (
                <div className="absolute right-0 mt-3 w-56 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-slate-800 py-2 z-50">
        ...
        {/* Content bg stays consistent with body or slightly different for depth */}
        <main className="flex-1 overflow-y-auto p-4 md:p-8 bg-[#F8FAFC] dark:bg-slate-950 transition-colors duration-300">
...
function DropdownLink({ to, icon: Icon, label }: any) {
  return (
    <Link to={to} className="flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-gray-600 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800 hover:text-blue-600 dark:hover:text-blue-400 transition-all">
...
function NotificationItem({ icon: Icon, color, bg, title, time, desc }: any) {
  return (
    <div className="p-4 hover:bg-gray-50 dark:hover:bg-slate-800/50 border-b border-gray-50 dark:border-slate-800 cursor-pointer transition-colors">
      <div className="flex gap-3">
        <div className={`p-2 rounded-lg ${bg} ${color} h-fit shadow-sm`}><Icon className="w-5 h-5" /></div>
        <div className="flex-1">
          ...
          <p className="text-xs text-gray-500 dark:text-slate-400 line-clamp-1">{desc}</p>
        </div>
      </div>
    </div>
  );
